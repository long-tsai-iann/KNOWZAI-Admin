...
- **推播測試**（`/push-test`）：開發者用，確認 App 的 FCM 推播是否正常、
  有沒有明顯延遲——送到自己帳號名下已註冊的裝置，或指定單一裝置 token
- **緊急狀態調整**（`/emergency-status`）：物資分配站能不能被建立，預設由
  後端依地震規模/最大震度自動分級（3 小時／3 天／30 天），這裡讓管理員可以
  延長（自動視窗到了但救災還在進行）或提早結束（自動分級判斷過寬）
- **AI 助理開關**（`/ai-assistant`）：阿巧的 kill switch（主專案
  `docs/iso42001/03` 的 T-07）。發現有害回覆、供應商異常、金鑰疑似外洩時在這裡
  立即關閉；App 的對話 API 回 503、入口隱藏，其他功能不受影響，最多 15 秒生效。
